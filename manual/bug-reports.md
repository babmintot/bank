# Баг-репорты

---
** Актуальные баг-репорты находятся в разделе [Issues](https://github.com/babmintot/bank/issues) этого репозитория.**

---



## BR-01. Перевод невозможен при граничном значении суммы

**ID:** BR-01  
**Название:** Перевод недоступен, когда сумма перевода с комиссией равна доступной сумме  
**Серьёзность:** Major  
**Приоритет:** High  
**Тип дефекта:** Ошибка бизнес-логики / граничное значение

### Окружение
- Браузер: Google Chrome
- Способ запуска: локально через `python3 -m http.server 8000`
- URL: `http://localhost:8000/?balance=30000&reserved=20001`

### Предусловия
- Баланс счёта: `30000`
- Резерв: `20001`
- Доступная сумма: `9999`

### Шаги воспроизведения
1. Открыть приложение.
2. Нажать кнопку **«Рубли»**.
3. Ввести номер карты: `1111222233334444`.
4. Ввести сумму перевода: `9099`.

### Ожидаемый результат
- Комиссия должна составлять `900`.
- Общая сумма списания должна составлять `9999`.
- Так как сумма перевода с комиссией **не превышает** доступную сумму, перевод должен быть разрешён.
- Должна отображаться кнопка **«Перевести»**.

### Фактический результат
- Отображается сообщение: **«Недостаточно средств на счёте»**.
- Кнопка **«Перевести»** отсутствует.
- Перевод недоступен.

### Скриншот
![Граничный баг — перевод 9099](images/bug-boundary.png)

### Итог
Система некорректно обрабатывает граничное значение суммы перевода и ошибочно запрещает операцию.

---

## BR-02. Принимается номер карты длиной 17 цифр

**ID:** BR-02  
**Название:** Поле суммы отображается при вводе номера карты длиной более 16 цифр  
**Серьёзность:** Major  
**Приоритет:** High  
**Тип дефекта:** Ошибка валидации пользовательского ввода

### Окружение
- Браузер: Google Chrome
- Способ запуска: локально через `python3 -m http.server 8000`
- URL: `http://localhost:8000/?balance=30000&reserved=20001`

### Предусловия
Приложение открыто и доступно для ввода данных перевода.

### Шаги воспроизведения
1. Открыть приложение.
2. Нажать кнопку **«Рубли»**.
3. Ввести номер карты: `11112222333344445` (17 цифр).

### Ожидаемый результат
- Номер карты должен считаться невалидным.
- Переход к следующему шагу должен быть заблокирован.
- Поле ввода суммы не должно отображаться.

### Фактический результат
- Поле ввода суммы отображается.
- Пользователь может продолжить оформление перевода.

### Скриншоты
![17 цифр номера карты](images/bug-card-length.png)  
![17 цифр номера карты — доступно продолжение перевода](images/bug-card-length-2.png)

### Итог
Система некорректно валидирует длину номера карты и принимает значение, не соответствующее требованиям.

---

## BR-03. Разрешён ввод отрицательной суммы перевода

**ID:** BR-03  
**Название:** Поле суммы перевода принимает отрицательное значение  
**Серьёзность:** Major  
**Приоритет:** Medium  
**Тип дефекта:** Ошибка валидации пользовательского ввода / бизнес-логики

### Окружение
- Браузер: Google Chrome
- Способ запуска: локально через `python3 -m http.server 8000`
- URL: `http://localhost:8000/?balance=30000&reserved=20001`

### Предусловия
Приложение открыто и пользователь перешёл к вводу данных перевода.

### Шаги воспроизведения
1. Открыть приложение.
2. Нажать кнопку **«Рубли»**.
3. Ввести номер карты: `1111222233334444`.
4. Ввести сумму перевода: `-100`.

### Ожидаемый результат
- Отрицательная сумма должна считаться невалидной.
- Кнопка **«Перевести»** не должна отображаться.
- Пользователь не должен иметь возможность выполнить перевод с отрицательной суммой.

### Фактический результат
- Поле суммы принимает отрицательное значение.
- Отрицательная сумма не блокируется системой.
- Пользователь может продолжить сценарий перевода.

### Скриншоты
![Отрицательная сумма в переводе](images/bug-negative.png)  
![Отрицательная сумма в переводе — перевод доступен](images/bug-negative-2.png)

### Итог
В приложении отсутствует корректная валидация суммы перевода, что позволяет вводить отрицательные значения.

---

## BR-04. Кнопка перевода отображается без ввода суммы

**ID:** BR-04  
**Название:** Кнопка перевода появляется без ввода суммы перевода  
**Серьёзность:** Major  
**Приоритет:** High  
**Тип дефекта:** Ошибка валидации пользовательского ввода / бизнес-логики

### Окружение
- Браузер: Google Chrome
- Способ запуска: локально через `python3 -m http.server 8000`
- URL: `http://localhost:8000/?balance=30000&reserved=20001`

### Предусловия
Приложение открыто и пользователь перешёл к вводу данных перевода.

### Шаги воспроизведения
1. Открыть приложение.
2. Нажать кнопку **«Рубли»**.
3. Ввести номер карты: `1111222233334444`.
4. Не вводить сумму перевода.

### Ожидаемый результат
- До ввода суммы кнопка **«Перевести»** не должна отображаться.
- Пользователь не должен иметь возможность перейти к выполнению перевода без указания суммы.

### Фактический результат
- Кнопка **«Перевести»** отображается без ввода суммы.
- Пользователь может продолжить сценарий перевода, не указав сумму.

### Скриншот
![Кнопка перевода появилась без ввода суммы](images/bug-empty-amount.png)

### Итог
В приложении отсутствует корректная проверка обязательности поля суммы перевода.

**ID:** BR-05
**Название:** Перевод выполняется при пустом поле суммы
**Серьёзность:** Critical
**Приоритет:** High
**Тип дефекта:** Ошибка валидации пользовательского ввода

### Окружение
- Браузер: Google Chrome
- URL: http://localhost:8000/?balance=30000&reserved=20001

### Шаги воспроизведения
1. Нажать кнопку «Рубли»
2. Ввести номер карты: 1111222233334444
3. Поле суммы оставить **ПУСТЫМ** (удалить все цифры)
4. Нажать кнопку «Перевести»

### Ожидаемый результат
Появляется сообщение об ошибке: «Введите сумму перевода»
Перевод НЕ выполняется

### Фактический результат
Перевод выполняется (сумма считается как 0)
Появляется уведомление: «Перевод 0 ₽ на карту 1111222233334444 принят банком!»

### Скриншот
<img width="720" height="728" alt="2026-05-20_00-24-10" src="https://github.com/user-attachments/assets/2344cd30-b367-4986-8fbf-048cf6e3aae7" />

<img width="354" height="118" alt="2026-05-20_00-24-01" src="https://github.com/user-attachments/assets/9a7c306b-0730-44cb-aa0b-75cbcd82e94f" />

**ID:** BR-06
**Название:** Перевод выполняется при сумме 0 ₽
**Серьёзность:** Major
**Приоритет:** High
**Тип дефекта:** Ошибка валидации пользовательского ввода / бизнес-логики

### Окружение
- Браузер: Google Chrome
- URL: http://localhost:8000/?balance=30000&reserved=20001

### Шаги воспроизведения
1. Нажать кнопку «Рубли»
2. Ввести номер карты: 1111222233334444
3. Ввести сумму: **0**
4. Нажать кнопку «Перевести»

### Ожидаемый результат
Ошибка: «Сумма перевода должна быть больше 0»
Перевод НЕ выполняется

### Фактический результат
Перевод выполняется
Появляется уведомление: «Перевод 0 ₽ на карту 1111222233334444 принят банком!»

### Скриншот
<img width="355" height="116" alt="2026-05-20_00-24-33" src="https://github.com/user-attachments/assets/7f98264d-f28f-4d30-a9bc-04f13c97dcc1" />
<img width="775" height="714" alt="2026-05-20_00-24-47" src="https://github.com/user-attachments/assets/3d7b1392-c8e2-4ca1-9765-114f2087a841" />
